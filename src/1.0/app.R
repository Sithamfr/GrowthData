...
evol3d_feats = function(signal,period,h,couleur="blue"){ # h est le décalage pour se situer en minuit (debut de periode)
  ...
  plot_ly(M,
          ...
          y=~Energy,
          z=~HDR,
          line=list(color=~pente,
                    opacity = 0.4),
          text=~paste("Drift :",Pente,
                      "<br>PERIOD ",N_period),
          type = 'scatter3d',
          mode = 'lines')
}
    ...
    tabItems(
      tabItem(tabName="home",
              ...
              column(5,
                     ...
                     valueBox("Image analysis",
                              HTML("- Select an image of the sequence<br>
                                ...
                                - 3D decomposition of the image<br>
                                     - Create a segmentation mask of the plants<br>
                                   - Download your binary mask"),
                              icon=icon("image"), color="purple", width="60%"),
                     valueBox("Sequence analysis",
                              HTML("- Evolution of the area covered by plants<br>
                                  - 3D Evolution of the average color<br>
                                   - Donwload the evolution of the canopy area"),
              ...
              column(5,
                     ...
                     valueBox("Sequence analysis",
                              HTML("- Evolution of the distance with the camera<br>
                                  - Represent your chosen features over time<br>
                ...
                column(6,
                       box(width="100%",title="Extensive tools",status="primary",solidHeader=TRUE,
                           ...
                           pickerInput("choice_seg_or_decomp",
                                       choices=c("-  Select your visualisation tool  -","3D Histogram","Plants segmentation")),
...
server = function(input, output, session) {
  ...
  output$evol2d_area = renderPlot({
    ...
    evol = 100*apply(list_seg,2,mean)
    D = data.frame(Time=1:N,evol)
    ggplot(D,aes(x=Time)) + geom_line(aes(y=evol,col=evol),alpha=0.6,lwd=1.5) + ylim(0,100) +
      labs(y="Estimated plant surface (%)")+ guides(color=FALSE) +
    ...
    if (is.null(input$seq)){
      ...
      tagList(
        pickerInput("depth_seq_choice",
                    choices=c("3D evolution of the average color","Difference of growth between 2 times")),
...
output$ana_seq_depth_plot = renderUI({
  ...
  output$segmentation_twice = renderPlot({ # PENSER A METTRE UNE LEGENDE
    t1 = input$times_seg[1]
    t2 = input$times_seg[2]
    X1 = segmentation(values$sequence[,,,t1],TRUE)
    X2 = segmentation(values$sequence[,,,t2],TRUE)
  ...
  output$evol3d_meancol = renderPlotly({
    ...
    for (i in 1:N){
      ...
      mask = segmentation(values$sequence[,,,i])
      M = c()
      for (j in 1:3){
  ...
  output$explain_feats = renderUI({
    HTML("Color lines depend on the average growth for each period<br>
         <b>Average growth on periods :</b>
